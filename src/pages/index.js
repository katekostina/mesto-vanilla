...
const newPlacePopup = new PopupWithForm(
    '#placepopup', 
    {
        handleSubmit: (evt, { placename, placeurl }) => {
            ...
            api.postNewCard(placename, placeurl)
                .then(item => {
                    createCard(item.name, item.link, item.likes.length, item.owner._id, userInfo.getUserId(), item._id);
                })
                .catch((error) => {
                    console.log(error);
                })
...
const editProfilePopup = new PopupWithForm(
    '#editprofilepopup',
    {
        handleSubmit: (evt, { profilename, profilecaption }) => {
            ...
            api.patchUserProfile(profilename, profilecaption)
                .then(result => {
                    userInfo.setUserInfo(result.name, result.about);
                })
                .catch((error) => {
...
const confirmDeletePopup = new PopupWithForm('#confirmdeletepopup', 
    {
...
// avatar https://www.dropbox.com/s/6tdq8md0sqyrp84/PixelArtMe2.png?dl=1